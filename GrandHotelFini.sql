...
select A.Année, CHB_ID, NbOccAnnée
from (
select distinct Année, MAX(NbOccAnnée) over(partition by Année) NbOccMaxAnnée
from vwOccAnneeChambre) A
inner join vwOccAnneeChambre AC on A.NbOccMaxAnnée = AC.NbOccAnnée and A.Année = AC.Année
order by 1, 2
go
...
select distinct CONVERT(nvarchar(7), PLN_JOUR, 102) AnnéeMois, 
				SUM(CONVERT(int, CHB_PLN_CLI_RESERVE)) over(partition by CONVERT(nvarchar(7), PLN_JOUR, 102)) * 100 /
				COUNT(1) over(partition by CONVERT(nvarchar(7), PLN_JOUR, 102)) [Taux Moy. Réservation]
from CHB_PLN_CLI
order by 1
go
...
select C.CLI_ID,  C.CLI_NOM, CONVERT(nvarchar(7), PLN_JOUR, 102) MoisSéjour, COUNT(1) NbJourSéjournéMois
from CHB_PLN_CLI CPC
...
-- 100 clients différents
-- 543 clients sans différencier les doublons
...
select distinct A.Année, COUNT(1) over(partition by Année) [Nb clients avec 2j suite 2015]
from(
	select TMP1.CLI_ID, YEAR(TMP1.PLN_JOUR) Année -- Rajouter un distinct si l'on ne souhaite avoir que le nombre de clients différents
	from vwCPC2015 TMP1
...
-- Clients par mois qui ont réservé plusieurs fois la même chambre sur le mois avec l'id de la chambre (pour l'affichage)
...
-- Clients de la table @ClientsResMmChMmMois qui ont réservé deux jours d'affilés sur le même mois
